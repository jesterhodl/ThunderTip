...
import {
    CommandSyntaxError, ConnectionRequestInGroupChatError,
    handleError,
    NegativeTipAmountError,
    ReceiverNotConnectedError,
    SenderConnectionError, UnexpectedCallbackQueryError,
    ZeroValueTipError
} from "../errors"
...
export async function handleInlineKeyboards(ctx:any){
    if(!ctx.callbackQuery.data){
        throw new Error("No callbackQuery data!")
    }
    switch(ctx.callbackQuery.data){
        case "connect":
            await ctx.conversation.enter("connectNWC");
        break;
        case "update":
            await ctx.conversation.enter("updateConnection");
            break;
        case "delete":
            await ctx.conversation.enter("deleteConnection");
            break;
        default:
            throw new UnexpectedCallbackQueryError("I don't know what you've clicked!")
    }
